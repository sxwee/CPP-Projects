...
万维网通过**HTTP（HyperText, Transfer Protocol，超文本传输协议）**工作。在HTTP中，客户端与服务器间建立连接，然后发送请求。例如通过`GET`请求斯坦福大学的主页http://www.stanford.edu/，在该过程中，首先浏览器需要与服务器www.stanford.edu建立连接，然后发送`GET`请求，然后服务器接收请求是否合法，用户是否能获取，然后回复响应。响应会带有一个数字码，例如`200`表示响应成功，`404`表示响应失败，没找到用户请求的文件。

------

**BitTorrent**

BitTorrent允许人们共享和交换大文件。在BitTorrent中，客户端从其他客户端请求文档，因此**单个客户端可以并行地向许多其他客户端请求数据**。
