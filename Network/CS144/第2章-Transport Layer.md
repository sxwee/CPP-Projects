...
流控制指**不要发送超过接收方处理能力的数据包**。为了达到这一目的，接受方需要给接收方反馈。
...
停等协议指任何时候**最多只能有一个数据包**在传输途中。接收方收到数据时会发送确认数据包。收到确认消息后，发送方会发送新的数据。当超时时，发送方会重传当前的数据。

停等协议中发送者和接收者对应状态转移如下：

<p align="center"><img src="Images/2-19-Stop-and-Wait-FSM.png" alt="2-19-Stop-and-Wait-FSM"></p>

停等协议下数据包发送可能出现的四种情况：

<p align="center"><img src="Images/2-20-stop-and-wait-examples.png" alt="2-20-stop-and-wait-examples"></p>

**情况一**：一切正常，数据包被正确送达，同时接收方回复了ack数据包。

**情况二**：发送的数据包丢失，此时发送方迟迟未收到ack确认，因此会在等待一段时间后重新发送该数据包。

**情况三**：数据包被正常接收，但回复的ack包丢失，此时发送方会等待一段时间后重新发送该数据包。

**情况四**：数据包被正常接收，但回复的ack包在网络中被延迟且超过timeout时间，此时发送方会重新发送该数据包，并接收到之前的延迟的ack包，然后发送方会发送新的数据包，此时接收者不知道该ack哪个数据包（重传的还是新的）。

## 7.2 滑窗

虽然停等协议可靠性较高，但由于只允许发送一个数据包，效率很低，尤其在高延迟、高带宽网络中更容易导致资源浪费。

相比之下，滑动窗口协议可以更灵活地控制数据流量。它**允许发送多个数据包，并充分利用网络带宽，提高了传输效率**。同时，当出现网络拥塞或丢包时，滑动窗口协议可以通过调整窗口大小和重传机制进行恰当的控制，保证数据的正确性和完整性，从而实现更加可靠的数据传输。另外，滑动窗口协议还可以利用**累积确认机制**，即接收方只需要发送最后一个连续接收的数据包的编号作为确认信息，无需每个数据包都返回确认信息，从而减少了网络开销和通信延迟。

滑窗的实现主要依赖于TCP报头中的**窗口**字段，通过该字段接收方可以向发送方表明自己能接收的最大字节数。

当发送方发送数据时，它会计算出当前窗口内未被确认的数据量，然后将这些数据发送出去。如果接收方成功收到数据，则会向发送方发送确认信息，同时移动自己的窗口，扩大自己的可接收范围。如果接收方的窗口大小为0，则表示接收方暂时无法接收数据，此时发送方需要停止发送数据，等待接收方重新打开窗口。

# 8.可靠通信

TCP在数据包丢失时需要重传。计算机网络中两种经典的重传策略为：

- **Go-back-N**：一个数据包的丢失会导致整个窗口需要进行重传。
- **Selective repeat**：一个数据包的丢失只会导致该数据包需要进行重传。
